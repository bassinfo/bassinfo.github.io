  ...
  window.jQueryFretboard.FretboardModel = function(settings, validator) {
    ...
    function setClickedNotes(clickedNoteGroups, asUser) {
      ...
      clickedNoteGroups.forEach(function(clickedNoteGroup) {
        clickFrettedNotes(
          clickedNoteGroup.string,
          clickedNoteGroup.notes,
          asUser
        );
      });
    }
    ...
    function clearClickedNotes() {
      model.clickedNotes = jQueryFretboard.utilities.createEmptyArrays(
        model.tuning.length
      );
    }
    ...
    function createClickedNotes() {
      model.clickedNotes = [];
      jQueryFretboard.utilities.pushMany(
        model.clickedNotes,
        jQueryFretboard.utilities.createEmptyArrays(model.tuning.length)
      );
    }
    ...
    function getIntervalByLetterAndRoot(letter, root) {
      ...
      var intervalIndex =
        letterIndex - rootIndex + (letterIndex >= rootIndex ? 0 : 12);
    ...
    function validateFrettedNoteGroup(frettedNoteGroup) {
      frettedNoteGroup.notes.forEach(function(frettedNote) {
        validator.validateFrettedNote(
          frettedNote,
          frettedNoteGroup.string,
          model.tuning,
          model.numFrets
        );
      });
    ...
    function clickFret(string, fret, asUser) {
      for (var i = 0; i < model.tuning.length; i++) {
        if (!jQueryFretboard.utilities.notesAreEqual(model.tuning[i], string))
          continue;
    ...
    function clickFrettedNoteAsUser(stringIndex, fret) {
      ...
      if (model.isChordMode) {
        clickFrettedNoteAsUserInChordMode(
          stringIndex,
          fret,
          fretAlreadyClicked
        );
      } else {
        clickFrettedNoteAsUserInScaleMode(
          stringIndex,
          fret,
          fretAlreadyClicked,
          indexOfClickedFret
    ...
    function clickFrettedNoteAsUserInChordMode(
      stringIndex,
      fret,
      fretAlreadyClicked
    ) {
    ...
    function clickFrettedNoteAsUserInScaleMode(
      stringIndex,
      fret,
      fretAlreadyClicked,
      indexOfClickedFret
    ...
    function updateClickedNotesForTuning(oldTuningLength) {
      ...
      if (numNewStrings < 0) {
        ...
        jQueryFretboard.utilities.pushMany(
          model.clickedNotes,
          jQueryFretboard.utilities.createEmptyArrays(numNewStrings)
        );
      }
    }
    ...
    function filterClickedNotes(stringIndex) {
      model.clickedNotes[stringIndex] = model.clickedNotes[stringIndex].filter(
        function(fret) {
...
(function() {
  ...
  window.jQueryFretboard.FretboardValidator = function() {
    ...
    function validateNote(note, noteLetters) {
      ...
      if (noteLetters.indexOf(note.letter) === -1) {
        throw new Error(
          "Note " +
            objectToString(note) +
            " is not in alNoteLetters: " +
            objectToString(noteLetters)
        );
      }
      ...
      if (!stringFound) {
        throw new Error(
          "The note: " +
            ...
            " that does not exist in the tuning: " +
            objectToString(tuning)
        );
      }
    }
    ...
    function validateNoteLetters(noteLetters) {
      if (!noteLetters) {
        throw new Error(
          "noteLetters is not valid: " + objectToString(noteLetters)
        );
      }
      ...
      if (Object.keys(hash).length !== 12) {
        throw new Error(
          "There must be 12 unique letters in noteLetters: " +
            objectToString(noteLetters)
        );
      }
    }
    ...
    function validateTuning(tuning, noteLetters) {
      ...
      if (!tuning.length) {
        throw new Error(
          "Tuning must have at least one note: " + objectToString(tuning)
        );
      }
      ...
      if (Object.keys(hash).length !== tuning.length) {
        throw new Error(
          "Tuning must contain unique notes: " + objectToString(tuning)
        );
      }
    }
    ...
    function validateIntervals(intervals, noteLetters) {
      ...
      if (Object.keys(hash).length !== 12) {
        throw new Error(
          "There must be 12 unique intervals: " + objectToString(intervals)
        );
      }
    }
    ...
    function validateNoteMode(noteMode) {
      if (!(noteMode === "letter" || noteMode === "interval")) {
        throw new Error(
          'Note mode must be "letter" or "interval": ' +
            objectToString(noteMode)
        );
      }
    }
...
(function($) {
  ...
  window.jQueryFretboard.FretboardHtmlRenderer = function(
    settings,
    $element,
    validator
  ) {
    ...
    var stringContainerContainerSelector =
      "." + stringContainerContainerCssClass;
    ...
    function $syncFretboard(allNotes) {
      ...
      $syncStringContainers(
        $getStringContainerContainer(),
        $getStringContainers(),
        oldNumStrings,
        oldNumFrets,
        fretboardBodyHeight
      ...
      $syncFretLines(
        $getFretLineContainer(),
        $getFretLines(),
        oldNumFrets,
        fretboardBodyWidth
      );
      $syncNoteCircles(
        $getNoteCircleContainer(),
        $getNoteCircles(),
        oldNumFrets,
        fretboardBodyWidth,
        fretboardBodyHeight
    ...
    function $postTimeoutRedraw(shouldBeAnimated, onPreRender) {
      ...
      var dimensions = model.dimensionsFunc(
        $fretboardContainer,
        $fretboardBody,
        model
      );
      ...
      $positionFretboardBody(
        fretboardBodyWidth,
        fretboardBodyHeight,
        shouldBeAnimated
      );
      $positionFretLines(fretWidth, shouldBeAnimated);
      $positionStringContainers(
        fretboardBodyHeight,
        fretWidth,
        shouldBeAnimated,
        shouldBeAnimated
    ...
    function $clickNote($stringContainers, string, frettedNote) {
      for (var i = 0; i < model.tuning.length; i++) {
        ...
        $clickFretOnString(
          $stringContainers.eq(i),
          frettedNote.fret,
          frettedNote.cssClass
        );
    ...
    function $getNote($stringContainer, fret) {
      return $stringContainer.find(noteSelector).eq(fret);
    }
    ...
    function $addOrEditStringContainer(
      $stringContainerContainer,
      $stringContainers,
      stringIndex,
      frets,
    ...
    function $editNote($oldNote, newNoteData) {
      ...
      $oldNote
        .find(noteDisplaySelector)
        .text($newNote.find(noteDisplaySelector).text());
    ...
    function $syncNoteCircles(
      ...
      for (var i = 0; i <= maxFrets; i++) {
        $syncNoteCircle(
          ...
          i,
          noteCirclesHash,
          noteCirclesThatShouldExistHash,
          fretboardBodyWidth,
          fretboardBodyHeight
        );
      }
    ...
    function $syncNoteCircle(
      ...
      if (fret <= model.numFrets) {
        $updateNoteCircleOnExistingFret(
          $noteCircleContainer,
          fret,
          noteCirclesHash,
          noteCirclesThatShouldExistHash,
    ...
    function $updateNoteCircleOnExistingFret(
      $noteCircleContainer,
      fret,
      noteCirclesHash,
      noteCirclesThatShouldExistHash,
    ...
    function $appendNoteCircle(
      ...
      if (type === "top") {
        position.top =
          getDoubleNoteCircleTopValue(fretboardBodyHeight, true) -
          $noteCircle.outerHeight(true) / 2;
      } else if (type === "bottom") {
        position.top =
          getDoubleNoteCircleTopValue(fretboardBodyHeight, false) -
          $noteCircle.outerHeight(true) / 2;
      } else {
        position.top =
          getMiddleNoteCircleTopValue(fretboardBodyHeight) -
          $noteCircle.outerHeight(true) / 2;
      }
    ...
    function $positionFretboardBody(
      fretboardBodyWidth,
      fretboardBodyHeight,
      shouldBeAnimated
    ) {
    ...
    function $positionStringContainers(
      fretboardBodyHeight,
      fretWidth,
      stringContainersShouldBeAnimated,
      notesShouldBeAnimated
    ) {
      var firstStringDistanceFromTop =
        fretboardBodyHeight / (model.tuning.length * 4);
      var fretHeight =
        (fretboardBodyHeight - firstStringDistanceFromTop * 2) /
        (model.tuning.length - 1);

      $getStringContainers()
        .removeClass(firstCssClass)
        .removeClass(lastCssClass)
        .each(function(stringIndex, stringContainer) {
          $positionStringContainer(
            stringIndex,
            $(stringContainer),
            fretWidth,
            fretHeight,
            firstStringDistanceFromTop,
            stringContainersShouldBeAnimated,
            notesShouldBeAnimated
          );
        });
    ...
    function $positionStringContainer(
      stringIndex,
      $stringContainer,
      fretWidth,
      fretHeight,
    ...
    function $positionNotes(
      ...
      $stringContainer.find(noteSelector).each(function(fret, note) {
    ...
    function $positionNote(
      ...
      var fretLeftVal = fret * fretWidth;
      var noteLeftVal =
        fretLeftVal + (fretWidth / 2 - $note.outerWidth(true) / 2);
    ...
    function $positionNoteCircles(
      ...
      Object.keys($existingNoteCircles).forEach(function(fret) {
        $positionNoteCirclesForFret(
          $existingNoteCircles[fret],
          fret,
          fretboardBodyHeight,
          fretWidth,
          shouldBeAnimated
    ...
    function $positionNoteCirclesForFret(
      $noteCircles,
      fret,
      fretboardBodyHeight,
      fretWidth,
      ...
      if ($noteCircles.length === 2) {
        $positionoDoubleNoteCirclesForFret(
          $noteCircles,
          fret,
          fretboardBodyHeight,
          fretWidth,
        ...
        $positionSingleNoteCircleForFret(
          $noteCircles[0],
          fret,
          fretboardBodyHeight,
          fretWidth,
    ...
    function $positionSingleNoteCircleForFret(
      $noteCircle,
      fret,
      fretboardBodyHeight,
      fretWidth,
      ...
      var position = {
        top:
          ...
          $noteCircle.outerHeight(true) / 2,
        left:
          fret * fretWidth + (fretWidth / 2 - $noteCircle.outerWidth(true) / 2)
      };

      $setPosition($noteCircle, position, shouldBeAnimated);
    }

    function $positionoDoubleNoteCirclesForFret(
      $noteCircles,
      fret,
      fretboardBodyHeight,
      fretWidth,
      ...
      var position1 = {
        top:
          getDoubleNoteCircleTopValue(fretboardBodyHeight, true) -
          $noteCircles[0].outerHeight(true) / 2,
        left:
          fret * fretWidth +
          (fretWidth / 2 - $noteCircles[0].outerWidth(true) / 2)
      };
      var position2 = {
        top:
          getDoubleNoteCircleTopValue(fretboardBodyHeight, false) -
          $noteCircles[1].outerHeight(true) / 2,
        left:
          fret * fretWidth +
          (fretWidth / 2 - $noteCircles[1].outerWidth(true) / 2)
      };
    ...
    function $setPosition($element, position, shouldBeAnimated) {
      var isVisible =
        $element.css("display") !== "none" && $element.css("opacity") > 0;
    ...
    function getFretHeight(fretboardBodyHeight) {
      return (
        (fretboardBodyHeight -
          getFirstStringDistanceFromTop(fretboardBodyHeight) * 2) /
        (model.tuning.length - 1)
      );
    }
    ...
    function getFretHeightFactorForNoteCircle() {
      ...
      var numberOfIncrements = Math.ceil(
        (model.tuning.length - maxStringNumForBaseline) /
          offsetAtWhichToIncrement
      );
    ...
    function getDoubleNoteCircleTopValue(fretboardBodyHeight, isTopCircle) {
      if (isTopCircle) {
        return (
          getFirstStringDistanceFromTop(fretboardBodyHeight) +
          getFretHeightFactorForNoteCircle() *
            getFretHeight(fretboardBodyHeight)
        );
      }
    ...
    function noteDataAreEqual(note1, note2) {
      return (
        jQueryFretboard.utilities.notesAreEqual(note1, note2) &&
...
(function($) {
  ...
  $.fn.fretboard = function(options) {
    return this.each(function() {
      ...
      var defaultDimensionsFunc = function(
        $fretboardContainer,
        $fretboardBody,
        settings
      ) {
      ...
      function setClickedNotes(clickedNoteGroupsWithCssClasses, asUser) {
        clickedNoteGroupsWithCssClasses = $.extend(
          true,
          [],
          clickedNoteGroupsWithCssClasses
        );
        ...
        reattachCssFromExistingDomNotesOntoAllNewModelNotes(
          allNotes,
          clickedNoteGroups,
          $clickedNoteGroups
        );
        reattachCssFromAllNewModelNotes(
          clickedNoteGroups,
          clickedNoteGroupsWithCssClasses
        );
      ...
      function createFretboardModel() {
        ...
        fretboardModel = new jQueryFretboard.FretboardModel(
          modelSettings,
          validator
        );
      }
      ...
      function createFretboardRenderer() {
        ...
        fretboardRenderer = new jQueryFretboard.FretboardHtmlRenderer(
          fretboardRendererSettings,
          $element,
          validator
        );
      }
      ...
      function createCompleteDimensionsFunc(fn) {
        return function($fretboardContainer, $fretboardBody, model) {
          var defaultDimensions = defaultDimensionsFunc(
            $fretboardContainer,
            $fretboardBody,
            model
          );
          ...
          var dimensions =
            fn === defaultDimensionsFunc
              ? defaultDimensions
              : fn($fretboardContainer, $fretboardBody, model);
      ...
      function reattachCssFromExistingDomNotesOntoAllNewModelNotes(
        allNewModelNotes,
        newModelNotes,
        $existingNotes
      ) {
        newModelNotes.forEach(function(newModelNoteGroup) {
          reattachCssFromExistingDomNotesOntoNewModelNotes(
            allNewModelNotes,
            newModelNoteGroup.notes,
            newModelNoteGroup.string,
            $existingNotes
          );
        });
      }

      function reattachCssFromExistingDomNotesOntoNewModelNotes(
        allNewModelNotes,
        newModelNotesOnString,
        string,
        $existingNotes
      ) {
        newModelNotesOnString.forEach(function(newModelNoteOnString) {
          reattachCssFromExistingDomNotesOntoNewModelNote(
            allNewModelNotes,
            newModelNoteOnString,
            string,
            $existingNotes
          );
        });
      ...
      function reattachCssFromExistingDomNotesOntoNewModelNote(
        allNewModelNotes,
        newModelNote,
        string,
        $existingNotes
        ...
        for (var i = 0; i < $existingNotes.length; i++) {
          ...
          var existingModelNote =
            allNewModelNotes[existingNoteData.stringIndex].notes[
              existingNoteData.fret
            ];
          var existingModelString =
            allNewModelNotes[existingNoteData.stringIndex].string;

          if (
            !jQueryFretboard.utilities.frettedNotesAreEqual(
              newModelNote,
              string,
              existingModelNote,
              existingModelString
      ...
      function reattachCssFromAllNewModelNotes(
        newModelNotesWithoutCss,
        newModelNotesWithCss
      ) {
        newModelNotesWithoutCss.forEach(function(newModelNoteWithoutCss) {
          reattachCssFromNewModelNotes(
            newModelNoteWithoutCss.notes,
            newModelNoteWithoutCss.string,
            newModelNotesWithCss
          );
        });
      }

      function reattachCssFromNewModelNotes(
        newModelNotesWithoutCss,
        string,
        newModelNotesWithCss
      ) {
        newModelNotesWithoutCss.forEach(function(newModelNoteWithoutCss) {
          reattachCssFromNewModelNote(
            newModelNoteWithoutCss,
            string,
            newModelNotesWithCss
          );
        });
      ...
      function reattachCssFromNewModelNote(
        newModelNoteWithoutCss,
        string,
        newModelNotesWithCss
      ) {
        var modelNoteFound = findModelNote(
          newModelNoteWithoutCss,
          string,
          newModelNotesWithCss
        );
      ...
      function findModelNote(modelNoteToFind, string, modelNotesToSearch) {
        for (var i = 0; i < modelNotesToSearch.length; i++) {
          var found = findFrettedNote(
            modelNoteToFind,
            string,
            modelNotesToSearch[i].notes,
            modelNotesToSearch[i].string
          );
      ...
      function findFrettedNote(
        noteToFind,
        stringForNoteToFind,
        notesToSearch,
        stringForNotesToSearch
      ) {
      ...
      function onUserNoteClick(e, clickedNoteDomEl) {
          ...
          [
            {
              ...
              notes: [
                allNotes[clickedNoteData.stringIndex].notes[
                  clickedNoteData.fret
                ]
              ]
            }
          ],
        ...
        reattachCssFromExistingDomNotesOntoAllNewModelNotes(
          allNotes,
          newClickedNoteGroupsFromModel,
          fretboardRenderer.getClickedNotes().$elements
        );
